...
struct SpendingSummaryRow: View {
    ...
    var body: some View {
            ...
            GeometryReader{ g in
                ...
                ZStack{
                    HStack{
                        Rectangle().frame(width: (g.size.width - 20) * CGFloat(self.viewData.percentage!), height: 45).foregroundColor(Color(.systemFill)).cornerRadius(10)
                        Spacer()
                    }
                    
                        
                    HStack{
                        Text(self.viewData.icon).font(.headline).padding(.leading).padding(.leading)
                        ...
                        Text(self.viewData.displayPercent).foregroundColor(Color(.white)).padding(.trailing)
